...
export class TokenService {
  ...
  async refreshTokens( userId: number, accessToken: string, refreshToken: string ): Promise<void> {
    await this.pool.query(
        'UPDATE tokens SET access_token = $2, refresh_token = $3 ' +
        'WHERE user_id = $1',
        [userId, accessToken, refreshToken],
    );
  }
  ...
  async findTokensByUsername(username: string): Promise<any> {
    const result = await this.pool.query(
        'SELECT * FROM tokens ' +
  ...
  async deleteTokensByUsername(username: string): Promise<void> {
    await this.pool.query(
        'DELETE FROM tokens ' +
        'WHERE user_id = (SELECT id FROM users WHERE username = $1)', [username]);
  }
